...
- **near-duplicate / repeat-passage finder** (surface the repeat-masking signal as a report);
...
### 4.6 Honesty constraint on every Wave 0 analytic
...
- **FR-7 — Dependency check.** A consumer declares a typed requirement; the resolver reuses a compatible layer or fails loud with a descriptive reason. `self_similarity` is migrated to consume a chunk layer + embedding layer via this mechanism instead of inline computation.
...
- **NFR-1 — Backward compatibility.** Existing analyses and their on-disk artifacts continue to work; behavior is byte-identical at existing defaults until a user opts into the new flow. `self_similarity`'s public results are unchanged.
