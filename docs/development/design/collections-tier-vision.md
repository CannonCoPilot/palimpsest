...
- **Liftover.** Once an A↔B alignment exists, project A's masks / annotations / score-tracks onto B's coordinates via the alignment `OffsetMap`, producing new remapped track versions on B — additive, nothing destroyed (FR-42).
...
- **FR-29 — Cross-text masking.** Corpus-repeat masking and low-correspondence masking (§3.4) apply via the existing excise/remap `OffsetMap` path; the masking policy is declared, reported, and never a silent mutation.
- **FR-30 — Cross-text & collection tracks.** A cross-text similarity layer remaps operand→root and renders as a root-frame track via the existing lane loop (FR-13). Collection-level annotation tracks are declared once and applied per member (each remapped into its member frame).
...
- **FR-42 — Annotation / mask liftover.** Given an A↔B alignment, project A's masks / annotations / score-tracks onto B's coordinates via the alignment `OffsetMap`, producing new remapped track versions on B. Purely additive; built on the same remap discipline as analyzable→original (principles §4).
...
| Cross-text alignment = coordinate map | `OffsetMap` operand→root remap (analysis-design-principles §4.1) — no new coordinate math |
...
| Cross-text & collection tracks | the FR-13 `rendering`-descriptor lane loop + `OverviewBar` (root-frame tracks draw with no new renderer) |
