...
- **Cross-text masking** (corpus-repeat + low-correspondence) via the existing excise/remap `OffsetMap` path; declared, reported, never silent.
- **Cross-text similarity tracks** (remapped to a chosen root lens → ordinary root-frame tracks via the FR-13 lane loop) + **collection-level annotation tracks** (one declaration, per-member remap).
- **Liftover** (FR-42) — given an A↔B alignment, project A's masks / annotations / score-tracks onto B's coordinates via the alignment `OffsetMap`, producing new remapped track versions on B. Purely additive (the same remap discipline as analyzable→original, principles §4); nothing destroyed.
- **Done-criteria:** corpus-repeat layer renders on the overview; cross-text mask demonstrably changes a downstream alignment; a cross-text track draws on the root lens; a mask/track lifted A→B lands at correct B-coordinates as a new additive version; tests green.

### C6 — Corpus analyses, probe & scale  *(FR-31, FR-22, FR-35)*
- Formula/topos diffusion, near-duplicate/stemma, influence graph (over the C3 graph); corpus IDF/BM25 weighting.
- **Probe mode** `R(q, Corpus)` over the shared embedding space (`SqliteVecStore.search`).
- **Scale — candidate generation as a recall dial** (FR-35): small matrices default **exhaustive/dense** (no missed signal); large matrices *recommend* (never force) candidate generation — ANN over embeddings (HNSW `efSearch` / IVF `nprobe` / multi-probe LSH / over-fetch-then-rerank) or MinHash-LSH over shingles; the alignment family's dial is seed sensitivity. The mode spans **exhaustive ↔ high-recall ↔ fast** with an **estimated-recall readout** and a **forced-exhaustive escape hatch**; exact scoring runs on candidates → sparse top-k; never a silent cap (report what was pruned).
- **Compute infrastructure:** parallel pair evaluation (worker pool), tiled/streamed matrices, MLX batch embedding; memory discipline (tiling, sparse top-k, memmap, dtype control); staged **%-progress** for long sweeps. Corpus-scale multi-stage sweeps press on the Wave-0 *"no persistent job DB"* non-goal — **revisit it here** if staged progress / resumability demands it.
- **Done-criteria:** corpus analyses produce results on a real collection; the recall dial prunes an O(N²) sweep with a reported estimated-recall and a working forced-exhaustive escape; a long sweep reports staged progress; tests green.
...
| Corpus-scale compute (parallelism, memory, long-sweep progress) outgrows the Wave-0 in-memory model | Med | Worker-pool parallel pairs, tiled / streamed / memmapped matrices, MLX batch; staged %-progress; **revisit the Wave-0 "no persistent job DB" non-goal** if resumability is required (C6). |
